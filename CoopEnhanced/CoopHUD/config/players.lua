...
ModConfigMenu.AddSetting(
	...
	{
		Type = ModConfigMenu.OptionType.NUMBER,
		CurrentSetting = function() return mod.Config.CoopHUD.players.twins.esau_display; end,
		Minimum = 0,
		Maximum = 2,
		Display = function() return "Better Esau Display: " .. (mod.Config.CoopHUD.players.twins.esau_display == 0 and "Disabled" or (mod.Config.CoopHUD.players.twins.esau_display == 1 and "Vanilla" or "Full")); end,
		OnChange = function(n) mod.Config.CoopHUD.players.twins.esau_display = n; end,
		Info = {"Disabled: Nothing is made transparent - Vanilla - Items are made transparent - Full: All of the twin HUD is transparent"},
	}
);
...
ModConfigMenu.AddSetting(
	...
	CoopHUD.MCM.categories.players,
	{
		...
		Minimum = 0.0,
		Maximum = 100.0,
		CurrentSetting = function() return tonumber(string.format("%.0f", mod.Config.CoopHUD.players.twins.esau_opacity * 100)); end,
		Display = function() return "Esau Opacity Reduction: " .. string.format("%.0f", mod.Config.CoopHUD.players.twins.esau_opacity * 100) .. "%"; end,
		OnChange = function(n) mod.Config.CoopHUD.players.twins.esau_opacity = n / 100; end,
		Info = {"Set what HUD opacity is lowered by when Better Jacob & Esau controls are enabled."},
	}
);
...
for i = 1, 4 do
	ModConfigMenu.AddSetting(
		...
		CoopHUD.MCM.categories.players,
		{
			...
			Info = "Character displays your character's name. (i.e. Isaac, Cain, etc). Custom requires using the command 'coop name hud <player_number> <player_name>'.",
		}
	);
end
ModConfigMenu.AddSpace(CoopHUD.MCM.title, CoopHUD.MCM.categories.players)
