...
function CoopHUD.RenderPlayers(screen_size, screen_center)
	if game:GetNumPlayers() > 0 then
		...
		for i = 1, game:GetNumPlayers(), 1 do
			...
			local edge = {pos = mod.Config.CoopHUD.offset + Vector.Zero,indexed = mod.Config.CoopHUD.offset + Vector.Zero,multipliers = Vector(1, 1)};
			
			if mod.Twins[i] then
				index = -i;
				num_twins = num_twins + 1;
				player_index = mod.Twins[i];
				edge.indexed = edge.indexed + mod.Config.CoopHUD.players.twins.offset;
				if mod.Config.CoopHUD.players.twins.pocket_offset and mod.CoopHUD.DATA.Players[player_index] and mod.CoopHUD.DATA.Players[player_index].Inventory.Pocket.Total > 0 then
					edge.indexed.Y = edge.indexed.Y + mod.CoopHUD.DATA.Players[player_index].Inventory.Pocket.Total * (16 * mod.Config.CoopHUD.pocket[(mod.CoopHUD.DATA.Players[player_index].Inventory.Pocket.Total - 1)].scale.Y);
				end
			else
			...
			if player_index == 0 or player_config == nil then break; end --Should never be Zero
			...
			if (player_index % 2) == 0 then
				edge.indexed.X = screen_size.Max.X - (edge.pos.X + mod.Config.CoopHUD.mirrored_extra_offset.X);
				edge.multipliers.X = -1;
			end
			if player_index > 2 then
				edge.indexed.Y = screen_size.Max.Y - (edge.pos.Y + mod.Config.CoopHUD.mirrored_extra_offset.Y);
			...
			Player.Render(index, screen_size, screen_center);
...
function CoopHUD.RenderBanners(screen_size, screen_center)
	...
	if not CoopHUD.DATA.Banner.Pos then 
		CoopHUD.DATA.Banner.Pos = screen_center;
		local anchor = mod.Config.CoopHUD.banner.anchor;
		local edge_multiplier = anchor == 1 and 1 or -1;
		if anchor > 0 then CoopHUD.DATA.Banner.Pos.Y = (anchor == 1 and mod.Config.CoopHUD.offset.Y or screen_size.Max.Y - mod.Config.CoopHUD.offset.Y) + ((50 * mod.Config.CoopHUD.banner.scale.Y) * edge_multiplier); end
...
function CoopHUD.RenderTimer(screen_size, screen_center)
	...
	if mod.CoopHUD.DATA.Timer.Visible then
		if CoopHUD.Refresh then
			...
			CoopHUD.DATA.Timer.Pos = screen_center;
			CoopHUD.DATA.Timer.Pos.X = screen_center.X - (mod.Fonts.CoopHUD.timer:GetStringWidth(CoopHUD.DATA.Timer.Value) / 2);
			if anchor > 0 then CoopHUD.DATA.Timer.Pos.Y = anchor == 1 and mod.Config.CoopHUD.offset.Y or screen_size.Max.Y - (mod.Config.CoopHUD.offset.Y + (16 * mod.Config.CoopHUD.misc.timer.scale.Y)); end
